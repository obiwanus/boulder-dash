#version 330 core

in VS_OUTPUT {
    vec2 tex_coord;
    vec3 normal;
    vec3 frag_pos;
} IN;

out vec4 Color;

uniform sampler2D dima;
uniform vec3 light_color;
uniform vec3 light_pos;

void main() {
    const float MAX_DISTANCE = 20.0;

    vec3 normal = normalize(IN.normal);
    vec3 light_direction = normalize(light_pos - IN.frag_pos);
    float light_distance = length(light_pos - IN.frag_pos);
    float distance_effect = min(mix(1.0, 0.1, light_distance / MAX_DISTANCE), 1.0);

    // Diffuse
    const float diffuse_strength = 0.3;
    float diffuse_impact = max(dot(normal, light_direction), 0.0);
    vec3 diffuse = diffuse_strength * diffuse_impact * light_color;

    // Ambient
    float ambient_strength = 0.2;
    vec3 ambient = ambient_strength * light_color;

    // Specular
    float specular_strength = 0.02;
    vec3 view_direction = normalize(-IN.frag_pos);
    vec3 reflection = reflect(-light_direction, normal);
    float spec = pow(max(dot(view_direction, reflection), 0.0), 32);
    vec3 specular = specular_strength * spec * light_color;

    Color = vec4(ambient + (diffuse + specular) * distance_effect, 1.0) * texture(dima, IN.tex_coord);
    // Color = texture(dima, IN.tex_coord);
}
